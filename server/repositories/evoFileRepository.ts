...
export async function getEvoFiles(
  ...
  if (ROLES_PROFESOR.includes(rol)) {
    const r = await queryPg<Record<string, unknown>>(
      `SELECT * FROM evo_files WHERE institution_id = $1 AND group_id = $2 ORDER BY updated_at DESC`,
    ...
    const r = await queryPg<Record<string, unknown>>(
      `SELECT * FROM evo_files
       WHERE institution_id = $1 AND group_id = $2
         AND (es_publico = true OR (es_publico = false AND propietario_id = $3))
  ...
  if (rol === 'padre') {
    const r = await queryPg<Record<string, unknown>>(
      `SELECT * FROM evo_files
       WHERE institution_id = $1 AND group_id = $2 AND es_publico = true
       ORDER BY updated_at DESC`,
      [institutionId, groupId]
    );
    return r.rows;
...
export async function createEvoFile(data: {
  ...
  const paramsWithCategory = [
    ...
    data.propietario_rol, data.es_publico, data.google_file_id ?? null,
